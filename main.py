...
    if len(parts) < 2:
        bot.send_message(message.chat.id, "Используй формат: /удали [фраза или номер]")
        return
    arg = " ".join(parts[1:])
    if arg.isdigit():
        index = int(arg) - 1
...
            memory.remove(arg)
            save_memory(memory)
            bot.send_message(message.chat.id, f"Удалил запись: {arg}")
        else:
            bot.send_message(message.chat.id, "Такой записи не найдено.")
...
            "Вот что я умею:\n\n"
            "*Память и дневник*\n"
            "/вспомни — что я запомнил\n"
            "/удали [фраза] — удалить запись\n"
            "/очисти — стереть всю память\n"
            "/запиши [текст] — сохранить мысль\n"
            "/дневник — показать дневник\n\n"
            "*Напоминания*\n"
            "/напомни [текст] через 10 минут\n"
            "/напоминания — список активных\n"
            "/отмени [текст] — удалить напоминание\n\n"
            "*Быстрое*\n"
            "/идея [текст] — сохранить идею\n"
            "/чувства [текст] — запомнить эмоцию\n"
            "/совет — попросить совет"
        )
...
        # Автозапоминание по ключевым словам
